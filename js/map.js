...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // --- City Data ---
  const cities = [
      { id: 'chennai', name: 'Chennai', x: 25, z: -15, isHQ: false, services: 'Premium Residential, Commercial', distance: '120 km', time: '2h 30m' },
      { id: 'walajah', name: 'Walajah', x: 15, z: -8, isHQ: false, services: 'Residential, Renovation', distance: '30 km', time: '45m' },
      { id: 'ranipet', name: 'Ranipet', x: 8, z: -2, isHQ: false, services: 'Residential, Commercial', distance: '15 km', time: '20m' },
      { id: 'arcot', name: 'Arcot', x: 11, z: -5, isHQ: false, services: 'Residential, Architecture', distance: '22 km', time: '30m' },
      { id: 'melvisharam', name: 'Melvisharam (HQ)', x: 4, z: 2, isHQ: true, services: 'All Services (Headquarters)', distance: '0 km', time: '0m' },
      { id: 'vellore', name: 'Vellore', x: -3, z: 6, isHQ: false, services: 'Premium Residential, Commercial', distance: '20 km', time: '30m' },
      { id: 'ambur', name: 'Ambur', x: -12, z: 12, isHQ: false, services: 'Residential, Turnkey', distance: '55 km', time: '1h 10m' },
      { id: 'vaniyambadi', name: 'Vaniyambadi', x: -18, z: 16, isHQ: false, services: 'Residential, Architecture', distance: '75 km', time: '1h 30m' },
      { id: 'bengaluru', name: 'Bengaluru', x: -28, z: 22, isHQ: false, services: 'Luxury Villas, Commercial', distance: '210 km', time: '4h 0m' },
    ];
  ...
  function init3D() {
    // Scene - Midnight Obsidian theme
    scene = new THREE.Scene();
    scene.background = new THREE.Color('#0A0A0E'); // Deep dark space/obsidian
    scene.fog = new THREE.FogExp2('#0A0A0E', 0.012);
    ...
    // Lighting - Cinematic Dark Setup
    const ambientLight = new THREE.AmbientLight(0x222233, 0.8); // Subtle moonlight blue ambient
    scene.add(ambientLight);
    
    // Main directional light (moonlight)
    const moonLight = new THREE.DirectionalLight(0x556699, 1.2);
    ...
    // Subtle edge rim light to highlight terrain features
  ...
  function createRealisticDiorama() {
    ...
    // Obsidian glass for natural terrain
    const baseMat = new THREE.MeshPhysicalMaterial({
      ...
      flatShading: true // Gives it a faceted crystal/obsidian look
    });
    ...
    // Subtle dark water layer in the valleys
    const waterGeo = new THREE.PlaneGeometry(140, 100);
    waterGeo.rotateX(-Math.PI / 2);
    const waterMat = new THREE.MeshPhysicalMaterial({
      color: 0x0A1A2A, // Deep abyssal blue
      transparent: true,
      opacity: 0.85,
      roughness: 0.05,
      metalness: 0.4,
    ...
    // Diorama solid base - Matte Graphite
    const boxGeo = new THREE.BoxGeometry(140, 10, 100);
    const boxMat = new THREE.MeshStandardMaterial({
      color: 0x030305, // Very dark graphite
      roughness: 1.0,
    });
  ...
  function createPhysicalRoute() {
    ...
    const orderedCities = [
      'chennai', 'walajah', 'ranipet', 'arcot',
      'melvisharam', 'vellore', 'ambur', 'vaniyambadi', 'bengaluru'
  ...
  function createInfoPanel() {
    ...
    panel.className = 'map-info-panel map-info-panel--premium';
  ...
  function focusCity(city) {
    ...
    panel.innerHTML = `
      <button class="map-info__close" id="map-info-close">
        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><line x1="18" y1="6" x2="6" y2="18"></line><line x1="6" y1="6" x2="18" y2="18"></line></svg>
      </button>
      <div class="map-info__content">
        <div class="map-info__header">
          <div class="map-info__title">${city.name}</div>
          <div class="map-info__subtitle" style="color: ${city.isHQ ? '#D4AF37' : '#aaaaaa'}">${city.isHQ ? 'Corporate Headquarters' : 'Service Region'}</div>
        </div>
        
        <div class="map-info__stats">
          <div class="map-info__stat">
            <div class="map-info__stat-label">Expertise</div>
            <div class="map-info__stat-value">${city.services}</div>
          </div>
          <div class="map-info__stat">
            <div class="map-info__stat-label">Proximity to HQ</div>
            <div class="map-info__stat-value">${city.distance} <span style="opacity: 0.5">(${city.time})</span></div>
          </div>
        </div>
        
        <div class="map-info__footer">
          <div class="map-info__price">Premium from <strong>₹1550*</strong>/sq.ft.</div>
          <a href="#contact" class="btn btn--gold btn--sm">Book Consultation</a>
        </div>
      </div>
    `;
    ...
    document.getElementById('map-info-close').addEventListener('click', (e) => {
      e.stopPropagation();
      resetMap();
    });
  }
  ...
  function animate() {
    ...
    if (!activeCity) {
      ...
      // Smoothly interpolate current camera position toward target parallax position
      // assuming base camera pos is INIT_CAMERA_POS
